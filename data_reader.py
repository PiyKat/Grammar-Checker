...
class DataReader(object):
    ...
    def __init__(self, config, train_path=None, token_to_id=None,
        ...
        self.config = config    ## Doubtful
        self.dataset_copies = dataset_copies    ### Doubtful
        ...
        if train_path is None:
            ...
            token_counts = Counter()       # A dictionary containing the
            ...
            count_pairs = sorted(token_counts.items(),
                                 key=lambda x: (-x[1], x[0]))    # Sort in descending order of frequency
            vocabulary, _ = list(zip(*count_pairs))
            vocabulary = list(vocabulary)
            # Insert the special tokens at the beginning.
            vocabulary[0:0] = special_tokens     # Adding special tokens(what tokens)
            self.full_token_to_id = dict(zip(vocabulary, range(len(vocabulary))))   # Dictionary containing unique id's to each word in the vocabulary
            self.token_to_id = dict(zip([k for k in self.full_token_to_id.keys() if self.full_token_to_id[k] <= max_vocabulary_size],
                                        [v for v in self.full_token_to_id.values() if v <= max_vocabulary_size]))

            #{k,self.full_token_to_id[k] for k in self.full_token_to_id.keys() if self.full_token_to_id[k] <= max_vocabulary_size}
            #self.token_to_id = self.full_token_to_id[:max_vocabulary_size] # Key - Token , values : id

        self.id_to_token = {v: k for k, v in self.token_to_id.items()} # Inverse of token-to-id
